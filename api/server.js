...
const jobs = new Map(); // jobId -> { status, progress, total, results, file, filename, error }

function createJob() {
  const jobId = Date.now().toString(36) + Math.random().toString(36).substr(2,6);
  jobs.set(jobId, { status:'pending', progress:0, total:0, enhanced:0, failed:0, results:[], file:null, filename:null, error:null, createdAt:Date.now() });
...
app.post('/api/excel/seo', rateLimit(3, 60000), upload.single('file'), async (req, res) => {
  try {
    ...
    if (!h4.includes('SEO')) {
      return res.status(400).json({ error: 'هذا ليس ملف SEO الصحيح' });
    }
    ...
    // إنشاء Job وإرجاع الـ ID فوراً
    const jobId = createJob();
    updateJob(jobId, { status:'processing', total:products.length });
    res.json({ jobId, total:products.length, message:'جاري المعالجة — تتبع التقدم عبر /api/excel/status/'+jobId });
...
[وصف SEO 140-160 حرف]`}]
              ...
              const text=msg.content[0].text;
              const ti=text.indexOf('###TITLE###'), di=text.indexOf('###DESC###');
              ...
              row.commit();
              return {status:'success',name:p.name,newTitle,newDesc};
            } catch(e) {
              if(attempt===2) return {status:'error',name:p.name,error:e.message};
            }
          }
        }
        ...
        for(let i=0; i<products.length; i+=BATCH) {
          ...
          const br=await Promise.all(batch.map(p=>processSeo(p)));
          for(const r of br) {
            results.push(r);
            if(r.status==='success') enhanced++; else failed++;
          }
          updateJob(jobId,{enhanced,failed,results:[...results]});
        }

        const buffer=await wb.xlsx.writeBuffer();
        updateJob(jobId,{
          status:'completed', enhanced, failed, results,
