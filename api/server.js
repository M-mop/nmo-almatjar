...
app.post('/api/generate-description', async (req, res) => {
  try {
    ...
    const message = await anthropic.messages.create({
      ...
      messages: [{
        role: 'user',
        content: `اكتب وصفاً احترافياً لمنتج على متجر سلة بالعربية:
المنتج: ${name}
الميزات: ${features}
...
اكتب:
1. عنوان SEO جذاب (60 حرف كحد أقصى)
2. وصف المنتج المقنع (150-200 كلمة)
3. نقاط البيع الرئيسية (5 نقاط)
4. الكلمات المفتاحية لـ SEO (10 كلمات)

اجعل الأسلوب مقنعاً يحفز على الشراء.`
      }]
    });
